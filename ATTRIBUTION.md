# Attribution

<!-- GENERATED FILE — do not edit by hand.
     Regenerate with `python build/build_attribution.py`.
     Hand edits are overwritten on every build. -->

## How to credit this database

If you use openexercisedb, credit it like this:

> Exercise data from [openexercisedb](https://github.com/rfivesix/openexercisedb),
> licensed under [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/),
> derived in part from the [wger project](https://wger.de).

## Upstream contributors

The initial dataset was imported from the wger project, whose exercise entries
carry per-entry licenses and per-entry authors.

Everything below this line is generated from the data itself — every record's
`upstream.license_author` — so that it stays correct as the database changes.
At this size a hand-maintained list is one that is guaranteed to be wrong
eventually. Attribution is owed to those individual contributors, not to "wger"
as a whole.

<!-- BEGIN GENERATED LIST -->
Across 909 exercises and 5546 translations, **249 distinct upstream authors** are credited. 937 of 6455 records carry no author upstream.
Additionally, **1119 descriptions** have been newly authored (`status: ai_authored`) where upstream descriptions were missing, placeholder-only, or fundamentally rewritten. Upstream attribution is preserved for exercise provenance.

### Original licenses

| License | Records |
|---|---|
| `CC-BY-SA-4.0` | 5589 |
| `CC-BY-SA-3.0` | 727 |
| `CC0-1.0` | 139 |

### Languages

| Language | Translations | Distinct authors | AI-authored |
|---|---|---|---|
| `ar` | 48 | 11 | 1 |
| `az` | 1 | 1 | 0 |
| `cs` | 51 | 15 | 7 |
| `de` | 908 | 140 | 289 |
| `el` | 48 | 11 | 0 |
| `en` | 909 | 240 | 360 |
| `eo` | 2 | 0 | 0 |
| `es` | 656 | 35 | 89 |
| `fa` | 3 | 2 | 0 |
| `fr` | 870 | 116 | 162 |
| `he` | 22 | 7 | 3 |
| `hr` | 31 | 8 | 5 |
| `id` | 48 | 11 | 1 |
| `it` | 868 | 239 | 171 |
| `ja` | 868 | 241 | 20 |
| `nl` | 49 | 12 | 1 |
| `pl` | 4 | 2 | 0 |
| `pt` | 66 | 18 | 1 |
| `ru` | 10 | 6 | 2 |
| `sv` | 3 | 2 | 0 |
| `tr` | 31 | 10 | 3 |
| `uk` | 2 | 1 | 0 |
| `zh` | 48 | 11 | 4 |

### Contributors

Listed by the name each contributor recorded upstream, with the number of records they authored. Sorted alphabetically, case-insensitively. Where someone recorded an email address, only the name part is shown — attribution is owed a name, not a mailbox.

- 2dPREtEZP221u68Akf0JImv5L48 (6)
- 3leh (6)
- 54str (8)
- 6LXBO (6)
- aahuja (12)
- abeworld (6)
- aboksz (10)
- abuono (6)
- admin (54)
- Aleid (5)
- almenociprovo (1)
- AlucardEvil40 (63)
- amaesc (20)
- amwang4 (4)
- Anastasious (39)
- andikeller (6)
- Anon#2 (16)
- anon1337 (11)
- anto.kreegyr (12)
- Antsy6277 (4)
- apeschel (15)
- arson (4)
- ataraxie67 (4)
- Awlex (6)
- baldurmen (5)
- barry (42)
- bbayuwega (4)
- beagle (24)
- Behrooz (4)
- bel (2)
- BeLikeWater (16)
- benjamin.yildiz (4)
- BePieToday (6)
- BFad07 (90)
- bizyguy (6)
- bl0sh (6)
- Blablabla (21)
- bobbyprince89 (5)
- Boertie (5)
- br_Orion (2)
- Bret Contreras (6)
- Brigade7938 (6)
- brucem (4)
- burenl (10)
- cal.zabel (19)
- captive0592 (52)
- carlos3c (20)
- Cerin (5)
- Cf (6)
- cgoob883 (5)
- clafal (159)
- cleen (10)
- colundrum (5)
- cozyGalvinism (4)
- CptnFatbeard (18)
- Croak6728 (138)
- cshep442 (16)
- cybro (42)
- cynomops (49)
- daiben (44)
- damnlost (3)
- daniel.escada (5)
- Davidgj32 (138)
- Deflation (12)
- delta@romeo (6)
- deusinvictus (6)
- Dexter (6)
- DiscoCop (6)
- djblitzd (4)
- donaddon (4)
- dookie1481 (6)
- drthurlow (10)
- Dulive (4)
- dvdfernandez95 (8)
- ege (24)
- Epiphany8424 (4)
- er0355 (14)
- erikocobra (36)
- eriktrinkle (6)
- eufvksruh (12)
- evtimovgeorg (12)
- Expenses7000 (102)
- ExRx (5)
- fabrice (4)
- FEFFO (16)
- Fittness69 (6)
- flanny (16)
- fletchgraham (4)
- flori (5)
- florian.bussmann (4)
- foguinho.peruca (5)
- Franpol (54)
- G. Johnson (6)
- Garion (2)
- Gavru (3)
- geraldbaeck (4)
- GiglioRosso (4)
- grilobosch (1)
- GrosseHund (12)
- Happy (28)
- hektkaso (25)
- HexoctusEngineering (42)
- hpmbala (72)
- hurr99 (12)
- Iko (4)
- Imobard (5)
- Imported from swim plan (40)
- Insight (8)
- intelman (6)
- ISSACS (6)
- J120290,cerin (4)
- JackSparrow (8)
- James Mackay (4)
- jayninja (6)
- jesusd (4)
- Jhonatan (7)
- jigglychipmunk (6)
- Joculari (5)
- JohnPreston (36)
- karly (16)
- klabautermann (6)
- kmcalderwood (4)
- Konsumopfer (13)
- koreyhinton (4)
- krisbbb (5)
- kwrindy (7)
- lakerbeezel (4)
- lauroernesto (4)
- lbroggi (24)
- LEBRERO (6)
- lh1701 (6)
- lhegedus (4)
- lion (4)
- lxmx (20)
- Lynn_McIntyre (42)
- m4k3r (10)
- M9 (70)
- MaddieBeasley (5)
- magdy (4)
- Mahoney (13)
- Manu, wikipedia (6)
- manuher89 (4)
- marcelbader (18)
- Mariano_O (14)
- Marius (5)
- matpn (6)
- mbozi1 (5)
- McMarcel13 (16)
- Me (6)
- meldun (16)
- Mens Fitness (9)
- Metin (4)
- Mike Mentzer (4)
- mike6426 (4)
- Mikko Ruohola (5)
- minifigmaster125 (16)
- MisterPinnacle (4)
- Moffi (80)
- mountain.potato (18)
- MrAlfaRobot (26)
- MrSteele (6)
- MU5H (4)
- Nallitnas (5)
- Nash (28)
- nate303303 (25)
- nishant0712 (20)
- notdefine (5)
- novadani (78)
- oboema (4)
- OGhTebfCxhexZXuf35mUxV9C--A (6)
- p177erpher (1)
- painDpice (18)
- Papazit (7)
- PaukOne (10)
- Paul@Chemistry (6)
- pera_perkan (6)
- personal use (1)
- Pete-E automation (41)
- Pete-E automation probe (1)
- philip (40)
- phpi (5)
- pjwirth (7)
- polloperro (34)
- powerade69 (5)
- prevail90 (4)
- probablyforging (6)
- pwiltrout (4)
- Raj (6)
- Rehab Hero (6)
- ricardodavidrd (4)
- RiccaBaro (8)
- richmr2174 (5)
- ricwheatley (4)
- Robertcoop (6)
- robhoyt (10)
- roneydya (8)
- Rottekongen (28)
- SAH (6)
- schweezer (10)
- schwolfar (5)
- sebk (6)
- Settebello (17)
- sevae (13)
- Shiladree (32)
- shushu (5)
- sistab2 (15)
- Skadi (6)
- sophialj (14)
- spacetowaste (6)
- sTiKyt (8)
- student1234 (20)
- stwalkerster (1)
- taylorbarbell (5)
- tdprice12 (5)
- tekknokrat (5)
- tenebrizz (10)
- teus_ergaster (20)
- ThisGirl0819 (6)
- Tierrasverdes (66)
- tinman (28)
- TobiasFalk (12)
- Torsten Linnecke (6)
- treder (4)
- tregga (4)
- Trix (4)
- trzr23 (6)
- tuckerm (61)
- tuninx (4)
- utkb (10)
- Vasco Custom (13)
- Vazco (6)
- vdrb (4)
- Vilhelmo (4)
- vince63 (6)
- vkylamba (4)
- wakanda90 (12)
- wampir5000 (3)
- wger.de (1752)
- wgerjhn (105)
- wgerpott (4)
- Whythebigpaws (4)
- WiNNiE (4)
- wintercyan (6)
- workout (4)
- xWan (7)
- Yderkone (4)
- YYCfit (4)
- YYCfit / BFad07 (18)
- zdelko (5)

### Sources

These were recorded upstream in the author field but are references to where an entry came from, not people.

- <http://www.bodybuilding.com/> (23)
- <http://www.carinatum.com/> (6)
- <http://www.exrx.net/WeightExercises/Brachialis/DBC> (5)
- <http://www.realsimple.com/health/fitness-exercise/> (5)

A further 937 records carry no author upstream and are attributed to *the wger project and its contributors*. They are counted here rather than dropped, so the gap stays visible.
<!-- END GENERATED LIST -->

## Note on missing authors

Where no author is recorded upstream, the record is attributed to *the wger
project and its contributors*. The generator counts those records rather than
dropping them, so the gap stays visible instead of being silently papered over.
