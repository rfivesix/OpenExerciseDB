#!/usr/bin/env python3
"""One-time import: wger snapshot -> `data/exercises/` + `data/i18n/`.

First half of the decomposed `create_wger_exercise_db.py`. The second half is
`build/build_db.py`. The crucial difference from the legacy script: here the
path ends in text files, not in a `.db`. From that point on, data maintenance
is done via pull requests rather than API calls.

What this script does **not** do: invent any fields. `modality`, `mechanic`,
`force_vector`, `movement_pattern`, `laterality`, `usage_tags`,
`tracking_type`, `primary_equipment`, `setup`, and `difficulty` are not
provided by wger, so they appear nowhere. A placeholder would be
indistinguishable from a genuine value — exactly the trap SCHEMA.md 11 warns
against. They are Phase 2. The raw values from which they can be derived later
(category and equipment) are stored under `upstream.source_fields`.

Three things the legacy script lost that are captured here:

* **License and author per translation** (SCHEMA.md 3b). wger licenses per
  entry; the previously distributed DB contained no attribution whatsoever.
* **The correct languages.** The legacy script had hard-coded and incorrect
  wger language IDs: 4 was assumed to be `fr`, 5 `it`, 8 `ja` — in fact they
  are `es`, `ru`, and `el`. Japanese does not exist in wger at all. The mapping
  now comes from `vocab/languages.yaml` and is checked against the snapshot.
* **All languages**, not just five.

Usage:

    python3 import/wger_to_yaml.py                       # current snapshot
...
EXERCISE_HEADER = """data/exercises/<id>.yaml — language-neutral facts.
Contains NO translatable text; that belongs in data/i18n/<lang>/<id>.yaml.

Automatically generated by import/wger_to_yaml.py from the wger snapshot.
From here on, this file is maintained by hand — the importer does not run again.
Classification fields (modality, tracking_type, primary_equipment, ...)
are intentionally absent: wger does not provide them, and guessing is worse than
empty. See SCHEMA.md 6 and 11."""

TRANSLATION_HEADER = """data/i18n/<lang>/<id>.yaml — text for exactly one exercise.

Automatically generated by import/wger_to_yaml.py from the wger snapshot.
License and author apply to this exact text, not to the whole repository — wger
licenses per entry (SCHEMA.md 3b)."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--snapshot", help="Path to snapshot (default: snapshot/current.json)")
    parser.add_argument(
        "--out-exercises", default=str(EXERCISES_DIR), help="Target directory for facts"
    )
    parser.add_argument("--out-i18n", default=str(I18N_DIR), help="Target directory for text files")
    parser.add_argument("--report-json-out", help="Path for machine-readable import report")
    parser.add_argument(
        "--report-max-examples",
        type=int,
        default=25,
        help="Maximum number of rejected examples in report (default: 25)",
    )
    parser.add_argument(
        "--prune",
        action="store_true",
        help="Delete target files no longer present in snapshot. Off by default: "
        "deletion is forbidden per SCHEMA.md 3; a missing upstream entry is "
        "handled as status: deprecated.",
    )
...
def clean_html(raw: Any) -> str:
    """Removes tags from a wger description field.

    Intentionally character-identical to the legacy script
    (`create_wger_exercise_db.clean_html`) so the generated DB in Phase 1
    provably contains the exact same text as the previously distributed one.
    This also means: HTML entities remain as-is (`&nbsp;` in 93 translations).
    Properly cleaning them up is a content change belonging in Phase 2.
    """
...
def ref_id(value: Any) -> Any:
    """wger delivers relations as an ID or as an object depending on endpoint."""
...
def build_language_map(vocab: Vocabularies, snap: snapshot_mod.Snapshot) -> dict[int, str]:
    """Registry -> wger language IDs, verified against snapshot.

    Aborts if the registry asserts something different from the API. This
    exact check was missing from the legacy script, causing Spanish texts to be
    served as French for years.
    """
    ...
    for wger_id, code in vocab.wger_language_ids.items():
        ...
        if actual is None:
            problems.append(f"  {code}: wger_language_id {wger_id} does not exist in the API")
        elif actual != code:
            problems.append(
                f"  {code}: wger_language_id {wger_id} is '{actual}' in API, not '{code}'"
            )
        else:
            mapping[wger_id] = code
    if problems:
        raise SystemExit(
            "vocab/languages.yaml contradicts snapshot:\n" + "\n".join(problems)
        )
    return mapping
...
def collect_muscles(
    ...
    """wger muscle values -> nodes in the new vocabulary.

    Primary wins: if the same node appears in both lists, only the primary
    assignment remains (Invariant 9). This does not occur in current data, but
...
def run(args: argparse.Namespace) -> tuple[int, dict[str, Any]]:
    ...
    for index, exercise in enumerate(snap.exercises):
        if not isinstance(exercise, dict):
            rejections.add("malformed_payload", raw_index=index, details="not an object")
            continue

        raw_id = exercise.get("id")
        exercise_id = normalize_text(str(raw_id)) if raw_id is not None else ""
        if not exercise_id:
            rejections.add("missing_required_source_fields", raw_index=index, details="missing id")
            continue
        ...
        if not isinstance(raw_translations, list):
            rejections.add("malformed_payload", id=exercise_id, details="translations is not a list")
            continue

        # ---- Collect texts by language code
        texts: dict[str, dict[str, Any]] = {}
        for translation in raw_translations:
            if not isinstance(translation, dict):
                rejections.add("malformed_payload", id=exercise_id, details="translation is not an object")
                continue
            ...
            if not name:
                continue
            if code in texts:
                # Does not occur in data; first translation wins so results do not depend on list order.
                continue
        ...
        # ---- Slug: from source language, fallback to first available
        ...
        if slug in slugs:
            # Deterministic and stable: ID is appended. Slugs are a contract per
            # SCHEMA.md 3 and cannot change later, so resolution must not depend on
            # encounter order.
        ...
        for code in sorted(texts):
            ...
            doc: dict[str, Any] = {
                ...
                "language": code,
                # Written by humans in the wger community, but never reviewed by
                # this project — and in 511 entries without author attribution.
                # 'human' would be read in six months as 'someone verified this';
                # the distinction would no longer be reconstructible.
...
def prune(
    ...
    """Removes files no longer in snapshot — only when explicitly requested."""
    stale: list[str] = []
...
def main() -> int:
    ...
    print(f"Exercises  {summary['imported_count']} of {summary['raw_exercise_count']} imported, "
          f"{summary['rejected_count']} rejected")
    print(f"Texts      {summary['translations_written']} in {summary['languages_written']} languages")
    for language, count in report["translations_per_language"].items():
        print(f"             {language}: {count}")
    print(f"Licenses   {report['licenses']}")
    print(f"Authors    {report['license_authors']['distinct']} distinct, "
          f"{report['license_authors']['without_author']} texts without author")
    print(f"Gaps       {gaps['without_any_muscle']} exercises without muscle, "
          f"{gaps['without_primary_muscle']} without primary")
    if gaps["unmapped_wger_muscles"]:
        print(f"           unmapped wger muscles: {gaps['unmapped_wger_muscles']}")
    if report["rejected_examples"]:
        print("Rejected   " + json.dumps(report["rejection_reasons"], ensure_ascii=False))
    if report["pruned_files"]:
        print(f"Orphaned   {len(report['pruned_files'])} files without snapshot entry")
